...
class Doge extends Fighter {
  constructor(scene, x, y, stats, isPlayer1) {
    ...
    if (this.scene.textures.exists('doge_atlas')) {
      this.setupDogeSprite(x, y, isPlayer1);
    } else {
  ...
  initializeDogeSkills() {
    console.log('Initializing Doge skills...');
    console.log('DogeSkill1 exists:', !!window.DogeSkill1);
    console.log('DogeSkill2 exists:', !!window.DogeSkill2);
  
    if (window.DogeSkill1 && window.DogeSkill2) {
      this.skills = {
        skill1: new window.DogeSkill1(this),
        skill2: new window.DogeSkill2(this)
      };
      console.log('Skills initialized:', this.skills);
    } else {
      console.error('DogeSkill1 or DogeSkill2 not found in window object');
      this.skills = {
        skill1: null,
        skill2: null
      };
  ...
  setupDogeSprite(x, y, isPlayer1) {
    ...
    // Buat sprite baru
    this.sprite = this.scene.add.sprite(x, y, 'doge_atlas', '{doge} #iddle 0.aseprite');
    ...
    // Play idle animation
    this.sprite.play('doge_idle', true);
    
    // Set fighter name
    this.fighterName = 'doge';
  }
  
  createAnimations() {
    try {
      const frameRate = 10;
      const atlasKey = 'doge_atlas';
      ...
      const effectKey = 'doge_skill_effect';
      ...
      console.error('Error creating Doge animations:', error);
    }
  }
  ...
  move(direction) {
    ...
    // Play walk animation
    this.sprite.play('doge_walk', true);
  ...
  attack(target) {
    ...
    this.sprite.play('doge_attack', true);
  ...
  jump() {
    ...
    // Play jump animation
    this.sprite.play('doge_jump', true);
  ...
  takeDamage(amount) {
    ...
    if (this.stats.hp > 0) {
      this.sprite.play('doge_hit', true);
  ...
  die() {
    // Play death animation
    this.sprite.play('doge_death', true);
  ...
  update(time, delta) {
    ...
    // Additional Doge-specific update logic can be added here
  }
  ...
  kick(target) {
    ...
    // Play kick animation
    this.sprite.play('doge_kick', true);
